document.addEventListener("DOMContentLoaded", () => {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsMarkup = participants.length
          ? `<div class="participants-list">${participants
              .map(
                (participant) => `
                  <div class="participant-row" data-participant="${participant}" data-activity="${name}">
  ...
  activitiesList.addEventListener("click", (event) => {
    ...
    const row = deleteButton.closest(".participant-row");
    const participantEmail = row?.dataset.participant;
